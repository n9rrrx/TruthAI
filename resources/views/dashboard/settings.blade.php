...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <!-- Profile Section -->
    <form action="{{ route('settings.profile') }}" method="POST" class="card rounded-2xl p-6" enctype="multipart/form-data">
        @csrf
        ...
        <div class="flex items-start gap-6 mb-6">
            ...
            <div class="relative group cursor-pointer" onclick="document.getElementById('avatar-input').click()">
                <div id="avatar-preview" class="w-20 h-20 rounded-2xl overflow-hidden">
                    @if($user->avatar)
                        <img id="current-avatar" src="{{ $user->avatar }}" alt="{{ $user->name }}" class="w-full h-full object-cover">
                    @else
                        <div id="avatar-placeholder" class="w-full h-full bg-gradient-to-br from-brand-primary to-brand-accent flex items-center justify-center text-white text-2xl font-bold">
                            {{ strtoupper(substr($user->name, 0, 1)) }}
                        </div>
                    @endif
                    <img id="avatar-image-preview" src="" alt="Preview" class="w-full h-full object-cover hidden absolute inset-0">
                </div>
                <!-- Hover overlay -->
            ...
            <div>
                ...
                <p class="text-xs text-slate-400 mt-1">Click avatar to change</p>
